...
import numpy as np
from dejavu.tests.dejavu_test import (
    DejavuTest,
    autolabeldoubles,
    generate_test_files,
    log_msg,
...
def main(
    ...
    # scan files
    log_msg(
        f"Running Dejavu fingerprinter on files in {src}...", log=log, silent=silent
    )

    tm = time.time()
    djv = DejavuTest(temp_folder, test_seconds)
    log_msg(
        f"finished obtaining results from dejavu in {(time.time() - tm)}",
        log=log,
        silent=silent,
    )
    ...
    all_query_duration = [
        [[0 for x in range(tests)] for x in range(djv.n_lines)] for x in range(n_secs)
    ]
    ...
    for sec in range(0, n_secs):
        ...
        means_dvj = [
            round(x[0] * 100 / div, 1) for x in all_matching_times_counter[sec]
        ]
        rects1 = ax.bar(ind, means_dvj, width, color="r")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=f"Runs a few tests for dejavu to evaluate "
    ...
    parser.add_argument(
        "-l", "--log", action="store_true", default=False, help="Enables logging."
    )
    parser.add_argument(
        "-sl",
        "--silent",
    ...
    parser.add_argument(
        "-sd", "--seed", action="store", default=None, type=int, help="Random seed."
    )
    parser.add_argument(
        "src", type=str, help="Source folder for audios to use as tests."
    )
